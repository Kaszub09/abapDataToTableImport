CLASS zcl_dtti_source_factory DEFINITION PUBLIC FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    CONSTANTS:
      BEGIN OF c_first_row_headers,
        ask_user    TYPE i VALUE 0,
        present     TYPE i VALUE 1,
        not_present TYPE i VALUE 2,
      END OF c_first_row_headers,
      BEGIN OF c_worksheet,
        ask_user  TYPE i VALUE 0,
        use_first TYPE i VALUE 1,
        use_name  TYPE i VALUE 2,
      END OF c_worksheet.
    ...
    CLASS-METHODS:
      ...
      "! @parameter file_path | <p class="shorttext synchronized" lang="en">Used if supplied, otherwise ask user</p>
      "! @parameter worksheet | <p class="shorttext synchronized" lang="en">Applicable only if file is XLSX and more than one worksheet found</p>
      "! @parameter worksheet_name | <p class="shorttext synchronized" lang="en">Applicable only if workhseet=c_worksheet-use_name</p>
      create_from_file IMPORTING file_path         TYPE string OPTIONAL
                                 worksheet         TYPE i DEFAULT c_worksheet-ask_user
                                 worksheet_name    TYPE string OPTIONAL
                                 first_row_headers TYPE i DEFAULT c_first_row_headers-ask_user
                       RETURNING VALUE(source)     TYPE REF TO zif_dtti_source.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS:
      ...
      get_data_table_from_excel IMPORTING file_path TYPE string worksheet TYPE i worksheet_name TYPE string
                                RETURNING VALUE(data_table) TYPE REF TO data,
      ...
      get_wks_name IMPORTING excel TYPE REF TO cl_fdt_xl_spreadsheet worksheet TYPE i worksheet_name TYPE string
                   RETURNING VALUE(wks) TYPE string,
...
CLASS zcl_dtti_source_factory IMPLEMENTATION.
  ...
  METHOD create_from_file.
    DATA(file_to_open) = COND #( WHEN strlen( file_path ) > 0 THEN file_path ELSE get_file_from_user( ) ).
    IF strlen( file_to_open ) = 0.
      RETURN.
    ENDIF.

    SPLIT file_to_open AT '.' INTO TABLE DATA(splitted_path).
    DATA(file_extension) = to_upper( splitted_path[ lines( splitted_path ) ] ).

    DATA(data_tab) = SWITCH #( file_extension
        WHEN 'XLSX' THEN get_data_table_from_excel( file_path =  file_to_open worksheet = worksheet worksheet_name = worksheet_name )
        WHEN 'TXT' OR 'CSV' OR space THEN get_data_table_from_txt( file_to_open ) ).
    IF data_tab IS NOT BOUND.
      RETURN.
    ENDIF.

    source = NEW zcl_dtti_source_table( source_table = data_tab is_first_row_headers = SWITCH abap_bool( first_row_headers
                WHEN c_first_row_headers-ask_user THEN ask_user( question = CONV #( TEXT-002 ) )
                WHEN c_first_row_headers-present THEN abap_true
                WHEN c_first_row_headers-not_present THEN abap_false ) ).
  ENDMETHOD.
  ...
  METHOD get_data_table_from_excel.
    ...
    DATA(wks) = get_wks_name( excel = excel worksheet = worksheet worksheet_name = worksheet_name ).
    IF strlen( wks ) = 0.
      RETURN.
    ENDIF.
  ...
  METHOD get_wks_name.
    ...
    IF lines( worksheets ) > 1.
      CASE worksheet.
        ...
        WHEN c_worksheet-use_first.
          wks = worksheets[ 1 ]-name.

        WHEN c_worksheet-use_name.
          wks = VALUE #( worksheets[ 1 ]-name DEFAULT space ).
